...
int ncpu[]=new int[n]; int pri[]=new int[n];
int finish[]=new int[100]; int turntt[]=new int[n]; int wait[]=new int[n];
int process[]=new int[n];
int t_quantum,difference,temp_sum=0,k=0; int seq[]=new int[100];
...
arrival[i]=s.nextInt();
System.out.println("Enter CPU time of "+(i+1)+" Process ncpu[i]=cpu[i]=s.nextInt();


process[i]=i+1;
 

System.out.println("Enter time quantum : "); t_quantum = s.nextInt();
...
System.out.println("Process execution sequence : "); while(sum!=temp_sum){
for(int i=0;i<n;i++)
{
if(ncpu[i]<t_quantum)
{
...
difference=ncpu[i]; tv=ncpu[i]; ncpu[i]=0;




difference = ncpu[i]-t_quantum; tv=t_quantum; ncpu[i]=difference;
 
if(tv > 0)
{
sum=sum+tv; finish[k]=sum; seq[k]=i;
System.out.print(seq[k]+1+" "); k++;
}
}
}
System.out.println();
...
for(int i=0;i<n;i++)
{
int carr=0,tt=0; carr=arrival[i];
...
if(seq[j]==i)
{
tt=tt+(finish[j]-carr); carr=finish[j];
}
}

turntt[i]=tt;
System.out.println("Turn around time for "+(i+1)+" process : "+turntt[i]);
total_tt=total_tt+turntt[i]; wait[i]=turntt[i]-cpu[i];
